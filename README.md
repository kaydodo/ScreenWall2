...
为解决服务端事件循环阻塞导致的"假死"问题，新增三项结合的健康自检机制：

1. **心跳检测**：主线程每秒更新心跳标记，检测事件循环是否阻塞
2. **健康检查端点**：`/_health` 端点供看门狗查询服务状态
3. **Worker Thread**：CPU密集操作（OCR、图像处理）移到独立线程，不阻塞主线程
4. **看门狗增强**：HTTP健康检查替代端口检查，检测假死并自动重启
...
┌─────────────────────────────────────────────────────────────┐
│                    看门狗（外部独立进程）                      │
│  每5秒 → HTTP GET /_health → 超时5秒                         │
│  200 OK → 继续监控                                            │
│  503/超时 → 连续2次失败 → 杀进程 → 重启服务                   │
└─────────────────────────────────────────────────────────────┘
                          ↓ HTTP 请求
┌─────────────────────────────────────────────────────────────┐
│                     服务端主线程                              │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │ WebSocket   │  │ HTTP请求    │  │ 心跳标记            │  │
│  │ 帧推送      │  │ /_health    │  │ setInterval每秒更新 │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
│                          ↓ 报警图像                           │
│              发送给 Worker Thread（不阻塞主线程）             │
└─────────────────────────────────────────────────────────────┘
                          ↓ postMessage
┌─────────────────────────────────────────────────────────────┐
│                  Worker Thread（独立线程）                    │
...
| 参数 | 值 | 说明 |
|------|-----|------|
| 心跳间隔 | 1000ms | setInterval 每秒更新心跳标记 |
| 心跳超时阈值 | 10000ms | 超过10秒无心跳判定为阻塞 |
| 看门狗HTTP超时 | 5秒 | 健康检查请求超时时间 |
| 看门狗失败阈值 | 2次 | 连续2次失败才重启服务 |

### 工作流程

**正常状态**：
```
主线程 setInterval 每秒更新 lastHeartbeat
看门狗请求 /_health → elapsed ≈ 1-2秒 → 返回 200 OK
报警图像 → Worker Thread处理 → 返回结果 → 主线程创建报警记录
```

**假死状态**：
```
主线程事件循环阻塞 → setInterval 无法执行 → lastHeartbeat 停止更新
看门狗请求 /_health → elapsed > 10秒 → 返回 503
看门狗连续2次收到503 → 杀进程 → 重启服务
```
...
| 文件 | 说明 |
|------|------|
| server/alarm-worker.js | Worker Thread，处理CPU密集的报警图像分析 |

### 修改文件

| 文件 | 修改内容 |
|------|----------|
| server/server.js | 添加心跳机制、健康检查端点、Worker Thread通信 |
| server/watchdog.ps1 | 从端口检查升级为HTTP健康检查 |

### 测试方法

```powershell
# 测试健康检查端点
Invoke-WebRequest -Uri "http://localhost:3000/_health" -UseBasicParsing

# 查看看门狗日志
Get-Content "$env:TEMP\sw_server_watchdog.log"
```
...
├── server/                          # 服务端
│   ├── server.js                    # 主服务程序
│   ├── alarm-worker.js              # 报警处理Worker Thread（v1.12.1新增）
│   ├── watchdog.ps1                 # 看门狗脚本（增强版，v1.12.1更新）
...
| | | | 删除废弃的 hq_1080 变量 |
| **v1.12.1** | 2026-06-03 | - | 服务健康自检机制： |
| | | | 心跳检测：每秒更新 lastHeartbeat，健康检查端点 /_health |
| | | | Worker Thread：分离报警图像处理到独立线程 |
| | | | 看门狗增强：HTTP健康检查替代端口检查，服务未运行时立即启动 |
| | | | 修复 logger 未定义错误：改为 serverLog/serverError |
| | | | 修复服务端自更新 bug：只写入原始字段，不写入动态添加的 uuVersion |
| **v1.13.0** | 2026-06-05 | - | 截图性能优化：解决周期性卡顿问题 |
| | | | DXGI 捕获器缓存：创建 DXGICapturer 类缓存 COM 链 |
| | | | MSS 捕获器缓存：ScreenCapturer 对象缓存，避免每帧重新初始化 |
| | | | 帧率节流优化：内网不节流（客户端已限制），外网降到 3fps |
| | | | PIL ImageGrab 保底方案无此问题（简单函数调用，无初始化开销） |
| **v1.13.1** | 2026-06-05 | - | 修复 MSS 缓存初始化问题 |
| | | | 初始化时 _capturer_monitor_index 设为 None，确保第一次创建截图器 |
| | | | 截图失败时重置截图器，下次循环重新创建 |
| | | | 解决副显示器长期无画面问题 |
| **v1.14.0** | 2026-06-06 | - | UU安装优化 + 显示器偏移量实时获取 |
| | | | UU安装进程检测优化：检测GameViewer.exe进程出现后再初始化 |
| | | | 移除全局缓存：_cached_monitor_offset改为实时获取 |
| | | | _get_current_monitor_offset()每次调用都重新查询显示器信息 |
| | | | 确保分辨率变化后坐标映射准确 |
| **v1.14.1** | 2026-06-06 | - | 客户端版本号更新 |
| | | | 客户端版本号同步到1.14.1 |
| **v1.14.2** | 2026-06-07 | - | 修复分辨率缓存未更新问题 |
| | | | 修复 _update_cached_monitor_offset 未定义错误 |
| | | | 移除已废弃的缓存更新函数 |
| **v1.14.3** | 2026-06-07 | - | 修复分辨率切换后画面错位 |
| | | | 切换1080P成功后自动清理截图器缓存 |
| | | | 自动重新连接注册，让服务端获取最新分辨率 |
| **v1.14.4** | 2026-06-07 | - | 优化分辨率切换逻辑 + 定时检测 |
| | | | 切换分辨率：只有真正切换成功时才重连注册 |
| | | | 添加每5秒定时检测分辨率变化 |
| | | | 变化时自动清理缓存并快速重新注册 |
| **v1.14.5** | 2026-06-07 | - | 快速注册机制 + 远控开关优化 |
| | | | **核心优化：新增 _fast_reregister_async() 函数** |
| | | | 保留WebSocket连接，直接发送register消息 |
| | | | 毫秒级响应，避免完全断开重连的数秒延迟 |
| | | | 远控开关优化：5处调用点全部改为快速注册 |
| | | | 1. 托盘菜单切换分辨率时 |
| | | | 2. 定时检测分辨率变化时 |
| | | | 3. 多显示器切换时 |
| | | | 4. 用户手动切换远控开关时 |
| | | | 5. 服务端发送 setKeyboardEnabled/Disabled 时 |
| | | | 画面丝滑流畅，不再有上下线延迟感 |
