# ScreenWall2 项目文档

## 项目概述

ScreenWall2 是一个多设备屏幕监控系统，支持同时监控多台安卓设备，实时显示画面并进行远程控制。

---

## 目录结构

```
D:\ScreenWall2\
├── server/                          # 服务端
│   ├── server.js                    # 主服务程序（Node.js）
│   ├── public/                      # 前端静态资源
│   │   ├── index.html              # 主页面（设备格子视图）
│   │   ├── main.html               # 主页面完整版
│   │   ├── monitor-wall.html       # 监控墙页面
│   │   ├── preview.html            # 预览弹窗页面
│   │   ├── self-service.html       # 自助登号页面
│   │   ├── style.css               # 样式文件
│   │   ├── config.json             # 配置文件
│   │   └── devices.json            # 设备数据持久化
│   ├── logs/                       # 日志目录
│   └── package.json                # 依赖配置
│
├── client/                          # 电脑客户端
│   ├── client.py                   # 客户端主程序
│   ├── config.json                 # 配置文件
│   ├── build_client.py             # 打包脚本
│   └── dist/                       # 打包输出
│
├── mumu-client/                     # MUMU模拟器客户端
│   ├── mumu_client.py             # 主程序
│   ├── injector49.exe              # 摄像头Hook注入器
│   └── config.json                 # 配置文件
│
├── mumu_camera_hook/               # MUMU摄像头Hook模块
│   ├── camera_hook49.cpp           # Hook DLL源码
│   ├── camera_hook49.dll           # Hook DLL
│   ├── injector49.cpp              # 注入器源码
│   ├── injector49.exe              # 注入器
│   ├── README.md                    # Hook模块文档
│   └── ANALYSIS_REPORT.md          # 技术分析报告
│
└── .trae/rules/                   # 项目规则
    ├── OFFLINE_HEAL_SPEC.md        # 离线自愈规范
    └── project_rules.md            # 项目规则
```

---

## 系统架构

### 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                              浏览器端                                       │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐     │
│  │  main.html   │ │monitor-wall  │ │ preview.html │ │self-service  │     │
│  │  (格子视图)  │ │  (监控墙)   │ │  (预览弹窗)  │ │ (自助登号)   │     │
│  └──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └──────┬───────┘     │
│         │                │                │                │               │
│         └────────────────┴────────────────┴────────────────┘               │
│                              │ WebSocket                                   │
└──────────────────────────────┼────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                            服务端 (server.js)                               │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                        WebSocket Server                              │   │
│  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐   │   │
│  │  │browserClients│ │wallClients │ │previewClients│ │wssClient   │   │   │
│  │  │  (浏览器)   │ │ (监控墙)   │ │ (预览)     │ │ (设备)    │   │   │
│  │  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                               │                                              │
│         ┌─────────────────────┼─────────────────────┐                        │
│         ▼                     ▼                     ▼                        │
│  ┌─────────────┐      ┌─────────────┐      ┌─────────────┐                  │
│  │  devices    │      │ 帧缓存      │      │ 业务逻辑    │                  │
│  │  (设备状态) │      │ _browserBatch│      │ 截图/报警   │                  │
│  └─────────────┘      │ _wallBatch  │      └─────────────┘                  │
│                       └─────────────┘                                       │
└─────────────────────────────────────────────────────────────────────────────┘
                               │
                               │ WebSocket
                               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                            设备客户端                                       │
│  ┌──────────────────────┐              ┌──────────────────────┐            │
│  │    电脑客户端         │              │   MUMU模拟器客户端    │            │
│  │    client.py         │              │   mumu_client.py     │            │
│  │                      │              │                      │            │
│  │  - 屏幕截图           │              │  - ADB截图           │            │
│  │  - 键盘鼠标控制       │              │  - WebSocket推流     │            │
│  │  - UU远程控制         │              │  - 摄像头Hook注入    │            │
│  └──────────────────────┘              └──────────────────────┘            │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 技术规格

### 画面分辨率与帧率

| 场景 | Level | 分辨率 | 说明 |
|------|-------|--------|------|
| 格子视图（main.html） | 0 | 480×270 | 最低画质，用于格子缩略图 |
| 监控墙格子（monitor-wall.html） | 1 | 853×720 | 中等画质 |
| 预览画面（preview.html） | 2 | 1280×720 | 高清画质 |
| 自助登号（self-service.html） | 无 | 360×640 | 专属通道，MUMU模拟器推荐使用540×960 |

**说明**：
- Level 0/1/2 由服务端根据订阅情况动态推送
- 自助登号页面使用固定专属通道，无需Level设置
- 推荐MUMU模拟器使用540×960分辨率以获得最佳帧率

### 帧类型定义

| 类型 | 值 | 方向 | 说明 |
|------|---|------|------|
| 0x01 | 客户端→服务端 | 客户端发送截图请求 | 用于获取指定level的截图 |
| 0x10 | 服务端→浏览器 | 实时画面帧 | WebP格式二进制帧 |

### 前端页面规格

| 页面 | 设备ID来源 | 主要功能 |
|------|-----------|----------|
| main.html | 设备注册 | 格子视图、UU控制、4-10列布局、分组筛选、收藏截图、报警记录、监控上墙 |
| monitor-wall.html | 设备注册 | 监控墙、浮动预览、自定义行列布局、快捷布局 |
| preview.html | URL参数/initData | 独立预览弹窗 |
| self-service.html | URL参数 | 自助登号、MUMU控制 |

### MUMU客户端规格

| 参数 | 值 |
|------|-----|
| 模拟器推荐分辨率 | 540×960 |
| 客户端输出分辨率 | 360×640 |
| 压缩格式 | WEBP（质量30） |
| 帧队列大小 | 3帧 |
| 帧超时 | 2秒 |
| 理论帧率 | 3-4 fps |

---

## 布局系统

### 主页面布局（main.html）

- **布局范围**：4-10列可选
- **默认布局**：4列
- **总格子数**：200个
- **特殊布局**：9列和10列为宽屏专用布局

| 列数 | 适用场景 |
|------|----------|
| 4列 | 标准桌面 |
| 5列 | 标准桌面 |
| 6列 | 标准桌面 |
| 7列 | 标准桌面 |
| 8列 | 标准桌面 |
| 9列* | 宽屏专用 |
| 10列* | 宽屏专用 |

### 监控墙布局（monitor-wall.html）

- **布局范围**：自定义行列
- **两套布局模式**：标准布局、接收布局
- **快捷布局**：4格、6格、9格、12格、16格、20格

| 布局 | 行列 | 说明 |
|------|------|------|
| 4格 | 2×2 | 最小布局 |
| 6格 | 3×2 | 紧凑布局 |
| 9格 | 3×3 | 完全平方 |
| 12格 | 4×3 | 紧凑布局 |
| 16格 | 4×4 | 完全平方 |
| 20格 | 5×4 | 紧凑布局 |
| 自定义 | N×M | 任意行列组合 |

---

## 筛选与标记功能

### 分组筛选

**功能描述**：
- 支持按设备分组进行筛选显示
- 可以多选分组，只显示属于选中分组的设备
- 提供"恢复默认"按钮，快速清除所有筛选

**操作步骤**：
1. 点击顶部"🔍 筛选"按钮打开筛选面板
2. 在分组列表中勾选要显示的分组
3. 格子视图会实时更新，只显示选中分组的设备
4. 点击"恢复默认"按钮清除所有筛选

**相关变量**：
```javascript
let groups = [];  // 分组列表 [{ id, name, color, deviceIds: [] }]
let activeFilterGroupIds = [];  // 当前激活的筛选分组ID
```

### 收藏标记

**功能描述**：
- 点击格子右上角的星星(★)可以标记设备为收藏
- 收藏状态持久化存储，刷新页面不丢失
- 方便快速定位重点监控设备

**操作步骤**：
1. 点击格子右上角的星星按钮
2. 星星变为黄色表示已收藏
3. 再次点击取消收藏

**截图功能**：
1. 点击顶部"★ 截图"按钮打开收藏截图面板
2. 面板会显示所有已收藏的设备
3. 点击"📷 截图"按钮为所有收藏设备批量截图
4. 截图保存在服务端，支持查看历史截图记录

**相关变量**：
```javascript
let favorites = [];  // 收藏设备列表 [{ deviceId, deviceName, groupId, cellIndex }]
let collections = [];  // 截图集合 [{ timestamp, items: [...] }]
```

---

## 报警系统

### 游戏掉线报警

**功能描述**：
- 设备掉线时自动生成报警记录
- 侧边栏显示报警面板，显示所有报警记录
- 支持标记报警为已查看
- 支持清除所有报警记录
- 报警记录24小时后自动删除

**报警面板功能**：
1. 显示报警设备名、时间、分组、格子位置
2. 新报警自动滚动到最底部
3. 已查看的报警会淡化显示
4. 点击"🗑️"按钮清除所有报警记录

**相关变量**：
```javascript
let alarmRecords = [];  // 报警记录列表
```

**报警记录结构**：
```javascript
{
  id: "xxx",  // 报警唯一ID
  deviceId: "xxx",  // 设备ID
  deviceName: "xxx",  // 设备名
  groupId: "xxx",  // 分组ID（可选）
  cellIndex: 0,  // 格子位置
  timestamp: Date.now(),  // 报警时间
  viewed: false  // 是否已查看
}
```

---

## 监控上墙

**功能描述**：
- 将选中的设备快速添加到监控墙
- 支持选择设备和布局配置
- 一键打开监控墙页面

**操作步骤**：
1. 点击顶部"📺 监控"按钮打开监控配置面板
2. 选择要上墙的设备
3. 配置监控墙行列数
4. 点击"🖥️ 上墙"按钮打开监控墙页面

---

## 通信协议

### WebSocket消息类型

#### 浏览器 → 服务端

| 消息类型 | 参数 | 说明 |
|----------|------|------|
| subscribePreview | deviceId | 订阅设备预览 |
| unsubscribePreview | deviceId | 取消订阅预览 |
| setLevel | deviceId, level | 设置流级别（0/1/2） |
| subscribeWall | cols, rows, devices[] | 订阅监控墙 |
| unsubscribeWall | - | 取消监控墙订阅 |
| keyClick | deviceId, key | 发送按键 |
| mouseClick | deviceId, x, y | 鼠标点击 |
| mouseSwipe | deviceId, x, y, x2, y2, duration | 鼠标滑动 |
| mouseScroll | deviceId, delta | 鼠标滚轮 |
| selfServiceInit | deviceId, deviceName | 自助登号初始化 |

#### 服务端 → 浏览器

| 消息类型 | 参数 | 说明 |
|----------|------|------|
| state | cells[], devices[], groups[], alarms[], favorites[], powerScenes[] | 初始状态 |
| deviceList | devices[] | 设备列表更新 |
| devicePreviewStatus | deviceId, status | 设备预览状态 |
| wallStateUpdate | devices[], groups[] | 监控墙状态更新 |
| screenshotBatch | screenshots[] | 截图批量推送 |
| latency | latency | 网络延迟 |
| alarm | alarm | 新报警 |
| alarmViewed | alarmId | 报警已查看 |
| alarmDeleted | alarmId | 报警已删除 |
| alarmRecords | records[] | 报警记录更新 |

#### 设备客户端 → 服务端

| 消息类型 | 参数 | 说明 |
|----------|------|------|
| register | deviceId, deviceName, version, screenWidth, screenHeight... | 设备注册 |
| binary (0x01) | deviceId, width, height, level, data | 截图请求响应 |

#### 服务端 → 设备客户端

| 消息类型 | 参数 | 说明 |
|----------|------|------|
| setLevel | level | 设置流级别 |
| keyClick | key | 按键事件 |
| mouseClick | x, y | 点击事件 |
| mouseSwipe | x, y, x2, y2, duration | 滑动事件 |

### 二进制帧格式

```
┌─────────────────────────────────────────┐
│  Header (8 bytes)                       │
├─────────────────────────────────────────┤
│ [0] 0x10 (frame type)                  │
│ [1] deviceId length                     │
│ [2] flags (bit0=HQ)                    │
│ [3] reserved                           │
│ [4-5] screen width (big-endian)        │
│ [6-7] screen height (big-endian)       │
├─────────────────────────────────────────┤
│ [8..8+n-1] deviceId (UTF-8)            │
├─────────────────────────────────────────┤
│ [8+n..] image data (WEBP)              │
└─────────────────────────────────────────┘
```

---

## 离线自愈机制

### 阈值定义

- **自愈阈值**：`FRAME_HEAL_THRESHOLD = 2`
- 设备离线后，收到连续2帧才恢复在线状态

### 自愈流程

```
设备离线 → 不更新画面 → 帧计数器++
                          ↓
                    计数器 ≥ 2?
                    ↓是          ↓否
               恢复在线      继续计数
               更新画面
```

### 自愈实现位置

| 文件 | 变量 | 类型 |
|------|------|------|
| main.html | frameHealCount | Map<deviceId, count> |
| monitor-wall.html | frameHealCount | Map<deviceId, count> |
| preview.html | frameHealCount | number |
| self-service.html | frameHealCount | number |

---

## 功能模块

### 1. 主页面 (main.html)

**功能**：
- 设备格子视图显示（200格，4-10列可选）
- 设备在线/离线状态管理
- UU远程控制
- 设备预览弹窗
- 分组筛选
- 收藏标记与截图
- 游戏掉线报警记录
- 监控上墙配置

**关键变量**：
```javascript
var FRAME_HEAL_THRESHOLD = 2;
var devices = {}; // deviceId -> device info
var frameHealCount = new Map(); // deviceId -> count
var groups = [];  // 分组列表
var alarmRecords = [];  // 报警记录
var favorites = [];  // 收藏列表
```

### 2. 监控墙 (monitor-wall.html)

**功能**：
- 监控墙布局显示（自定义行列）
- 浮动预览弹窗
- 设备切换
- 快捷布局：4/6/9/12/16/20格
- 两套布局模式：标准布局、接收布局

**关键变量**：
```javascript
var FRAME_HEAL_THRESHOLD = 2;
var wallPreviewDeviceId = null;
var wallData = { devices: [], groups: [], layout: 16 };
var compactLayouts = { 6: {rows:3,cols:2}, 12: {rows:4,cols:3}, 20: {rows:5,cols:4} };
```

### 3. 预览弹窗 (preview.html)

**功能**：
- 独立设备预览
- 键盘控制
- 设备列表选择

**关键变量**：
```javascript
var FRAME_HEAL_THRESHOLD = 2;
var deviceId = ''; // 从URL参数或sessionStorage获取
var fromWall = false; // 是否从监控墙打开
```

### 4. 自助登号 (self-service.html)

**功能**：
- MUMU模拟器画面预览（专属通道）
- 安卓三键控制（返回、主页、任务）
- 鼠标滑动、点击操作

**URL参数**：
```
?deviceId=xxx&deviceName=xxx
```

**关键变量**：
```javascript
var deviceId = null;        // 业务ID（电脑客户端）
var previewDeviceId = 'MUMU-service'; // MUMU设备ID（固定）
var FRAME_HEAL_THRESHOLD = 2;
```

### 5. MUMU客户端 (mumu_client.py)

**功能**：
- ADB截图（screencap -p）
- 图像压缩（WEBP 360×640）
- WebSocket实时推流
- 摄像头Hook自动注入

**关键参数**：
```python
FRAME_QUEUE_SIZE = 3
FRAME_SEND_TIMEOUT = 1.0  # 秒
FRAME_EXPIRE_TIME = 2.0   # 秒
COMPRESS_QUALITY = 30      # WEBP质量
OUTPUT_SIZE = (360, 640)  # 输出分辨率
```

### 6. 摄像头Hook (camera_hook49)

**功能**：
- 自动检测摄像头选择弹窗（336×316 Qt5窗口）
- 自动点击中心位置
- 命名管道通信

**管道名称**：`\\.\pipe\MuMuCameraHook`

---

## 配置文件

### server/public/config.json

```json
{
  "serverVersion": "1.9.3",
  "serverSelfUpdate": "0",
  "uuDownloadUrl": "/UURemote_Setup_xxx.exe"
}
```

### client/config.json

```json
{
  "server": {
    "host": "localhost",
    "port": 8080
  },
  "device": {
    "deviceId": "PC-001",
    "deviceName": "设备名称"
  }
}
```

### mumu-client/config.json

```json
{
  "adb": {
    "host": "127.0.0.1",
    "port": 16384
  },
  "server": {
    "host": "localhost",
    "port": 8080
  },
  "device": {
    "deviceId": "MUMU-service",
    "deviceName": "MUMU模拟器"
  }
}
```

---

## 部署指南

### 环境要求

- Node.js 16+
- Python 3.8+
- ADB (Android Debug Bridge)
- MUMU模拟器

### 启动服务端

```bash
cd D:\ScreenWall2\server
npm install
node server.js
```

服务地址：`http://localhost:8080`

### 启动电脑客户端

```bash
cd D:\ScreenWall2\client
python client.py
```

### 启动MUMU客户端

1. 启动MUMU模拟器（推荐分辨率：540×960）
2. 运行客户端：
```bash
cd D:\ScreenWall2\mumu-client
python mumu_client.py
```

---

## 内存管理

### 前端内存优化

| 优化项 | 实现位置 | 说明 |
|--------|----------|------|
| Blob URL释放 | 各页面 beforeunload | 防止图片缓存累积 |
| 定时器清理 | beforeunload | 防止定时器泄漏 |
| 帧队列限制 | mumu_client.py | maxsize=3，防止内存膨胀 |
| 帧超时丢弃 | 各页面 | age > 2秒丢弃旧帧 |

### 服务端内存优化

| 优化项 | 实现位置 | 说明 |
|--------|----------|------|
| 帧批量推送 | _browserBatch | 减少IPC压力 |
| 帧过期清理 | frameFrames | 超过2秒清理 |
| 连接清理 | ws.on('close') | 断开时清理Map/Set |

---

## 版本历史

### 早期版本

| 版本/阶段 | 特点 |
|------|------|
| 1.0 测试版 | 初始版本，Base64传输图片，性能较差 |
| Base64版本 | Base64编码传输，体积大，帧率低 |
| 二进制版本 | 图片改为二进制传输，使用WebP格式，体积缩小80%以上，帧率大幅提升 |

### 关键里程碑版本

| Git提交 | 版本 | 改进 |
|------|------|------|
| c7399e6 | 1.9.0 | 客户端L1/L2改用标准16:9比例，L1=853x480，L2=1280x720 |
| 836f3aa | - | 新增自助登号页面，客户端托盘菜单添加入口 |
| a1bd4b2 | - | 自助登号页面添加返回/主页按钮，客户端用简易窗口打开 |
| 4c19387 | 1.9.2 | 版本升级，完善自助登号功能 |
| 4940466 | 1.9.3 | 版本升级，完善自助登号功能 |
| f63df58 | - | 新增MUMU模拟器轻量级客户端和二维码识别功能 |
| f9ef998 | - | MUMU-client设备ID改为MUMU-service，分辨率480x854 |
| 9e35e25 | - | MUMU客户端截图分辨率改回360×640 |
| 9355a0a | - | MUMU客户端实现异步队列：截图与发送分离，超时丢弃旧帧 |
| 5977b38 | - | MUMU客户端自动检测并上报模拟器真实分辨率 |
| 37a1a11 | - | MUMU客户端跳过WEBP压缩发送PNG，前端支持PNG格式显示（测试） |
| 93ea774 | - | 恢复WEBP压缩版本 |
| 0c00852 | - | 优化MUMU客户端：移除性能日志，优化截图节奏 |
| a1bc897 | v49 | 新增摄像头Hook 49版本：自动注入+二次注入检测+命名管道预留 |
| b41afa4 | v49 | 完善自动注入+弹窗提示+文档更新 |
| fd4d0e5 | - | injector49改为GUI静默模式，无控制台窗口 |
| dfc9165 | - | 预览页设备列表弹窗加宽至380px自适应 |
| b776dd4 | - | 修复self-service.html内存泄露 |
| 60b7a14 | - | 完善README文档 |

### 摄像头Hook版本演进

| 版本 | Git提交 | 改进 |
|------|------|------|
| v35 | 2ddf5c6 | 添加详细帧分析 - 312字节HID报告结构分析 |
| v36 | c1634ce | Qt5摄像头选择弹窗Hook分析 |
| v37 | 49424ef | Qt5摄像头弹窗自动点击 |
| v38 | 69a21c4 | Qt5摄像头弹窗自动点击 - 使用BM_CLICK消息 |
| v49 | a1bc897 | 自动注入+二次注入检测+命名管道预留 |

### 当前版本

| 版本 | 日期 | 说明 |
|------|------|------|
| 1.9.3 | 2026-05-19 | 最新版本，自助登号功能、摄像头Hook v49、监控墙布局完善、分组筛选、收藏截图、报警系统 |

---

## 注意事项

1. **MUMU模拟器必须先启动**才能运行客户端
2. **推荐使用540P分辨率**以获得最佳帧率（3-4fps）
3. **摄像头Hook**会在客户端启动时自动注入
4. **设备ID**需全局唯一，避免冲突
5. **帧超时**设置为2秒，防止旧帧堆积
6. **主页面布局**：9列和10列为宽屏专用
7. **监控墙布局**：支持4/6/9/12/16/20格快捷布局及自定义行列
8. **报警记录**：24小时后自动删除
9. **收藏状态**：持久化存储，刷新页面不丢失
