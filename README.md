...
当前采用"状态保存/恢复"机制：
1. 在渲染前保存当前状态（滚动位置、输入框内容、选中状态）
2. 渲染后恢复之前保存的状态
3. 关闭弹窗时重置到默认位置

**涉及文件**：
- `server/public/main.html`：任务弹窗、配置中心相关函数
...
| 方案 | 描述 | 适用场景 |
|------|------|---------|
| **增量 DOM 更新** | 只更新变化的部分，而非整个区域 | 中等复杂度，不想引入框架 |
| **引入前端框架** | 使用 Vue/React 等框架的响应式系统 | 高复杂度，需要组件化 |
| **状态锁定机制** | 用户操作期间暂停自动刷新 | 简单场景，快速修复 |
