    ...
    for (const wallWs of wallClients.keys()) {
      ...
      if (layout) {
        ...
        if (resizeTasks.length > 0) {
          serverLog('[wallBatch] 准备发送到Worker, 任务数:', resizeTasks.length);
          const results = await imageWorkerPool.resizeBatch(resizeTasks);
          serverLog('[wallBatch] Worker返回结果数:', results.length);
  ...
  try {
    for (const browserWs of browserClients) {
      ...
      if (vpData && !wallClients.has(browserWs)) {
        ...
        if (resizeTasks.length > 0) {
          serverLog('[browserBatch] 准备发送到Worker, 任务数:', resizeTasks.length);
          const results = await imageWorkerPool.resizeBatch(resizeTasks);
          serverLog('[browserBatch] Worker返回结果数:', results.length);
          for (let i = 0; i < results.length; i++) {
            const r = results[i];
            const m = resizeMeta[i];
