...
HANDLE WINAPI HookedCreateFileW(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, 
    ...
    if (lpFileName) {
        ...
        if (IsCameraPath(path)) {
            char logBuf[1024];
            sprintf(logBuf, "CreateFileW CAMERA: '%s'", path);
            WriteLog(logBuf);
        }
    }
    ...
    if (lpFileName && hResult != INVALID_HANDLE_VALUE) {
        ...
        if (IsCameraPath(path)) {
            ...
            char logBuf[512];
            sprintf(logBuf, "Camera opened successfully! Handle=%p", hResult);
            WriteLog(logBuf);
        }
    }
...
BOOL WINAPI HookedReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, 
    ...
    if (IsCameraHandle(hFile)) {
        char logBuf[512];
        sprintf(logBuf, "ReadFile from camera: Handle=%p Size=%lu", hFile, nNumberOfBytesToRead);
        WriteLog(logBuf);
    }
...
BOOL WINAPI HookedWriteFile(HANDLE hFile, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, 
    ...
    if (IsCameraHandle(hFile)) {
        char logBuf[512];
        sprintf(logBuf, "WriteFile to camera: Handle=%p Size=%lu", hFile, nNumberOfBytesToWrite);
        WriteLog(logBuf);
    }
...
BOOL WINAPI HookedDeviceIoControl(HANDLE hDevice, DWORD dwIoControlCode, LPVOID lpInBuffer, 
    ...
    if (IsCameraHandle(hDevice)) {
        char logBuf[512];
        sprintf(logBuf, "DeviceIoControl camera: Handle=%p IOCTL=0x%08lX In=%lu Out=%lu", 
...
BOOL InstallHooks() {
    ...
    WriteLog("Hooks installed v21 - HID DEVICE TRACKING");
