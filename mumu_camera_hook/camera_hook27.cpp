...
static HANDLE g_Handles[MAX_HANDLES] = {0};
static int g_HandleCount = 0;

void AddHandle(HANDLE hFile) {
    for (int i = 0; i < g_HandleCount; i++) {
        if (g_Handles[i] == hFile) return;
    }
...
BOOL IsTrackedHandle(HANDLE hFile) {
    for (int i = 0; i < g_HandleCount; i++) {
        if (g_Handles[i] == hFile) return TRUE;
    }
    return FALSE;
}
...
BOOL WINAPI HookedDeviceIoControl(HANDLE hDevice, DWORD dwIoControlCode, LPVOID lpInBuffer, 
    ...
    if (IsTrackedHandle(hDevice)) {
        char logBuf[2048];
        sprintf(logBuf, "DeviceIoControl #%d: Handle=%p, IOCTL=0x%08lX", ++g_IoControlCount, hDevice, dwIoControlCode);
        WriteLog(logBuf);
    }
...
int WINAPI HookedlstrcmpW(LPCWSTR lpString1, LPCWSTR lpString2) {
    ...
    if (g_lstrcmpWCount++ < 200) {
        if (lpString1 && lpString2) {
            ...
            if (strstr(s1, "vid") || strstr(s1, "pid") || strstr(s1, "hid") || strstr(s1, "camera") ||
                strstr(s2, "vid") || strstr(s2, "pid") || strstr(s2, "hid") || strstr(s2, "camera")) {
                char logBuf[2048];
...
BOOL InstallHooks() {
    ...
    WriteLog("=============================================");
    WriteLog("Hooks installed v27 - Record ALL CreateFile");
    WriteLog("=============================================");
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID reserved) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        char logBuf[1024];
        sprintf(logBuf, "=============================================\nStats:\n  CreateFile=%d\n  ReadFile=%d\n  WriteFile=%d\n  DeviceIoControl=%d\n  lstrcmpW=%d\n  Handles=%d\n=============================================",
            g_CreateFileCount, g_ReadFileCount, g_WriteFileCount, g_IoControlCount, g_lstrcmpWCount, g_HandleCount);
        WriteLog(logBuf);
