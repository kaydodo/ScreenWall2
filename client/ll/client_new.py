            ...
            try:
                ...
                if hasattr(self, '_dxgi_w'):
                    ...
                    if rgb is not None:
                        ...
                        if hq and (pic.width > int(hq_limit * 2/3) or pic.height > hq_limit):
                            pic.thumbnail((int(hq_limit * 2/3), hq_limit), Image.LANCZOS)
                        buf = BytesIO()
                        if lossless:
        ...
        if img_bytes is None:
            try:
                if self._mss_ok and self._sct:
                    ...
                    if hq and (pic.width > hq_limit * 2/3 or pic.height > hq_limit):
                        pic.thumbnail((int(hq_limit * 2/3), hq_limit), Image.LANCZOS)
                    buf = BytesIO()
                    if lossless:
                        pic.save(buf, format="WEBP", lossless=True)
...
class ScreenWallClient:
    def __init__(self):
        ...
        self._mac_address = _get_mac_address()  # MAC 地址缓存
        # UU 设备信息获取状态
